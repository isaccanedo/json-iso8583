...
public class MessageTranslator extends Translator{
	...
	/**
	 * Analisa a mensagem ISO 8583 e converte-a em JSONObject usando especificações de configuração
	 * @param mensagem Mensagem ISO 8583 bruta
	 * @param formatMap Object contendo formato e variável
	 * @return Return JSON Object containing especificações comuns que existem na mensagem
	 */
	...
	public JSONObject parseISO8583(String message, JSONObject formatMap)
	{
		...
		{		
			...
			if(message.length() > 36)
			{
				...
				this.iso8583 = new ISO8583(type, formatMap);
				iso8583.parse(message);
				String[] arrFormat;
				try 
				{
					try
					{
						...
						{
							fieldName = "f"+idx;
							if(iso8583.hasField(idx))
							{
								subfield = iso8583.getValue(idx).toString();
	...
	/**
	 * Parse ISO 8583 message and convert it into JSONObject using configuration specs
	 * @param ISO8583 ISO8583 object instead of text
	 * @param formatMap Object containing format and variable
	 * @return Return JSON Object containing common specs that exists on the message
	 */
	public JSONObject parseISO8583(ISO8583 ISO8583, JSONArray formatMap)
	{
		return this.parseISO8583(ISO8583, this.arrayToObject(formatMap));
	}
	/**
	 * Parse ISO 8583 message and convert it into JSONObject using configuration specs
	 * @param ISO8583 ISO8583 object instead of text
	 ...
	 * @return Return JSON Object containing common specs that exists on the message
	 */
	public JSONObject parseISO8583(ISO8583 ISO8583, JSONObject formatMap)
	{
		JSONObject json = new JSONObject();
		if(!formatMap.equals(new JSONObject()))
		{		
	...
	{
		return this.buildISO85831(json, this.arrayToObject(formatMap), mti_id);
	}
	/**
	 * ISO 8583 message builder
	 * @param json Data package on common specs
	 ...
	 * @return Return byte contains ISO 8583 message
	 */
	public byte[] buildISO85831(JSONObject json, JSONObject formatMap, String mti_id)
	{
		this.iso8583 = new ISO8583(mti_id, formatMap);
		...
		try 
		{
			try
			{
			        ...
			        {			        	
				        ...
				        if(variable.length() > 0)
				        {
					        ...
					        if(field_type.equals("AMOUNT"))
					        {
					        	...
					        	iso8583.addValue(field, data, "AMOUNT", 12);
					        }
					        else if(field_type.equals("CNUMERIC"))
					        {
					        	...
					        	iso8583.addValue(field, data, "CNUMERIC", field_length+1);
					        }
					        else if(field_type.equals("DNUMERIC"))
					        {
					        	...
					        	iso8583.addValue(field, data, "DNUMERIC", field_length+1);
					        }
					        else if(field_type.equals("NUMERIC"))
					        {
					        	...
					        	iso8583.addValue(field, data, "NUMERIC", field_length);
					        }
					        else
					        {
					        	if((field_type.equals("LVAR") || field_type.equals("LLVAR") || field_type.equals("LLLVAR")) && !variable.contains(","))
					        	{
					        		...
					        		iso8583.addValue(field, data, field_type, field_length);
					        	}
					        	else
					        	{
					        		iso8583.addValue(field, data, field_type, field_length);
			    ...
			    while(iter.hasNext());
			    message = iso8583.toString().getBytes();
			}
			catch(Exception e)
			{
				e.printStackTrace();
	...
	public byte[] buildISO8583(JSONObject json, JSONObject formatMap, String mti_id)
	{
		...
		if(!formatMap.equals(new JSONObject()))
		{
			ISO8583 iso = new ISO8583(mti_id, formatMap);
				...
				try
				{
				    ...
				    do
				    {
				        ...
				        {			        	
					        ...
					        iso.addValue(field, data, field_type, field_length);
					        				        
				        }
				    }
				    while(iter.hasNext());
				    message = iso.toString().getBytes();
				}
				catch(Exception e)
				{
					e.printStackTrace();
